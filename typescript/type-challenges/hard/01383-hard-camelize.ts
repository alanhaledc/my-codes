...
type CamelizeStr<T> = T extends `${infer F}_${infer S}`
  ? `${F}${Capitalize<CamelizeStr<S>>}`
  : T;

type CamelizeArr<T> = T extends [infer F, ...infer R]
  ? [Camelize<F>, ...CamelizeArr<R>]
  : Camelize<T>;

type Camelize<T> = T extends Record<string, unknown>
  ? {
      [K in keyof T as CamelizeStr<K>]: T[K] extends unknown[]
        ? CamelizeArr<T[K]>
        : Camelize<T[K]>;
    }
  : T;
