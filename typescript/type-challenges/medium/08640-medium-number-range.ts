...
type BuildArray<
  T extends number,
  Arr extends unknown[] = []
> = T extends Arr["length"] ? Arr : BuildArray<T, [...Arr, unknown]>;

type AddOne<T extends number> = BuildArray<T> extends [...infer R]
  ? [...R, unknown]["length"]
  : never;
...
type NumberRange<
  ...
  H extends number,
  Arr extends number[] = []
> = L extends H ? [...Arr, L][number] : NumberRange<AddOne<L>, H, [...Arr, L]>;
